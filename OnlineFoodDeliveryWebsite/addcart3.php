...
include_once('db_connect.php');
       ...
       {$quantity=1;}
       $fid=$_GET['f_id'];
       $other_res_food = "SELECT `username`, `f_id`, `r_id`, `quantity` FROM `cart` WHERE username='$user' && r_id != '3' "; 
       ...
       {
              $query="INSERT INTO `cart` (`username`, `f_id`, `r_id`, `quantity`) VALUES ('$user','$fid','3', '$quantity')";
              if($result= mysqli_query($con,$query)){
                     echo "<script>alert('product added to cart')</script>";
                     echo "<script>window.open('resturant3.php','_self')</script>";
                     
              }   
       }
       else            //there is some row for this f_id in table cart so updating the quantity only.
       {
              $check = "SELECT `order_per_user`,`total_orders` FROM `food` WHERE f_id='$fid' AND r_id='3'";
              ...
              if($f>$p){
                  $f=$p;
                  $updation="UPDATE `food` SET order_per_user='$p' WHERE f_id='$fid' AND r_id='3'";} 
